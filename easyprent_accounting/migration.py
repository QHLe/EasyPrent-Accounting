...
def migrate_database(
    ...
    *,
    ...
    try:
        ...
        return MigrationOutcome(report, backup_path, cutover)
    except MigrationValidationError as error:
        report = error.report
        report["backup_path"] = str(backup_path)
        report["activated"] = False
...
def restore_database(backup_path: str | Path, active_path: str | Path) -> RestoreOutcome:
    ...
    with _readonly_connection(backup) as saved:
        ...
        try:
            ...
            return RestoreOutcome(preserved, synced)
        finally:
            candidate.unlink(missing_ok=True)
